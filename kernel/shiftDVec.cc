...
#if 0 //BOCO: original code (replaced) 
        enterpairs
     (strat->P.p,strat->sl,strat->P.ecart,pos,strat, strat->tl);
#else //BOCO: replacement
      ...
      // posInS only depends on the leading term
      assume
        (getShift(strat->P.p, strat->P.getDVecAtIndex(0)) == 0);
      strat->enterS(strat->P, pos, strat, strat->tl);
#if 0
      ...
      if (strat->P.lcm!=NULL)
{
  ...
  //konstructors for TObjects not compatible with singulars
  //philosophy
  ...
  if (TEST_OPT_WEIGHTM)
  {
    ...
    if (ecartWeights)
    {
      loGriToFile("omFreeSize for (ADDRESS)ecartWeights in bba ", (pVariables+1)*sizeof(short),1024);
...
//} //BOCO: original code (deleted) see above
  ...
  if ((TEST_OPT_WEIGHTM)&&(F!=NULL))
  {
    ...
    //else
    {
      loGriToFile("omAlloc definition of ecartWeights in initBBA ",(pVariables+1)*sizeof(short),1024);
...
*/
      ...
      if ( shift < UINT_MAX && 
           ...
           !(sevT[j] & not_sev) &&
#endif //#if (HAVE_SEV > 3)
           !redViolatesDeg(t.p, tTemp.p, uptodeg) )
      {ret = j; break;}
#endif
/* BOCO: we have no debug version at the moment
#endif
...
*/
      if (withT)
      {
        ...
        deBoGriPrint( "Reducing with T", 256 );
        int j;
#if 0 //BOCO: original code (replaced)
